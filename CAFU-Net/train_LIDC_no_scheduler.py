...
from MCA_UNet_main.nets.CAUNet import CAUNet
from HmsU_Net_main.d2.HmsUnet_nn import HmsUnet
from DA_TransUNet.Architecture import DATransUNet
from DA_TransUNet.Architecture.DATransUNet import CONFIGS
from BRAU_Netplusplus_master.networks.bra_unet import BRAUnet
...
from ablation_experiment import Paranet

#ablation experiment2
from liuzhouwork_UNet import UNet1,UNet2,UNet3,UNet4_no_SD,UNet5_no_SG,UNet6_no_S,UNet7_no_G
...
def parse_args():
    ...
    parser.add_argument('--name', default="M2SNet",
                        help='model name: UNET',choices=['UNET', 'SAR_UNet', 'UNet2P','UNet3P', 'swinunet', 'UCTransNet',"resunetpp"
                                                         ...
                                                         ,"Unet_Conformer","Unet_CFFM_Conformer","Paranet","CAUNet","HmsUnet","UNet1","UNet2","UNet3","UNet4_no_SD",
                                                         "UNet5_no_SG","UNet6_no_S","UNet7_no_G", "DATransUNet", "BRAUnet"])
...
def main():
    ...
    os.makedirs('checpoint/liuzhou_LIDC/{}'.format(file_name),exist_ok=True)
    ...
    #save configuration
    with open('checpoint/liuzhou_LIDC/{}/config.yml'.format(file_name), 'w') as f:
        yaml.dump(config, f)
    ...
    elif config["name"] == "Paranet":
        model = Paranet.Unet(1, 1)
    elif config["name"] == "CAUNet":
        model = CAUNet()
    elif config["name"] == "HmsUnet":
        model = HmsUnet(num_classes=1, in_chans=1)
    elif config["name"] == "UNet1":
        model = UNet1.Unet(1,1)
    elif config["name"] == "UNet2":
        model = UNet2.Unet(1,1)
    elif config["name"] == "UNet3":
        model = UNet3.Unet(1,1)
    elif config["name"] == "UNet4_no_SD":
        model = UNet4_no_SD.Unet(1, 1)
    elif config["name"] == "UNet5_no_SG":
        model = UNet5_no_SG.Unet(1, 1)
    elif config["name"] == "UNet6_no_S":
        model = UNet6_no_S.Unet(1, 1)
    elif config["name"] == "UNet7_no_G":
        model = UNet7_no_G.Unet(1, 1)
    elif config['name'] == "DATransUNet":
        config_vit = CONFIGS["R50-ViT-B_16"]
        model = DATransUNet.DA_Transformer(config_vit, img_size=128, num_classes=1)
    elif config['name'] == "BRAUnet":
        model = BRAUnet(img_size=128, in_chans=1, num_classes=1, n_win=8)
    else:
    ...
    for epoch in range(config['epochs']):
        ...
        log.to_csv('checpoint/liuzhou_LIDC/{}/log.csv'.format(file_name), index=False)

        trigger += 1

        if val_log['dice'] > best_dice:
            torch.save(model.state_dict(), 'checpoint/liuzhou_LIDC/{}/best.pth'.format(file_name))
